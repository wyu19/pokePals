...
/**
 * Normalize ALL sprites to exactly 90×85 pixels
 * 
 * Resize all idle/drag/eat/play sprites to 90×85, centered in 96×96 canvas
 */
...
async function main() {
  ...
  const finalBackupDir = path.join(__dirname, '../.gsd/milestones/M005/slices/S06/backup-before-90x85');
  ...
  for (const species of SPECIES) {
    ...
    for (const action of actions) {
      ...
      for (let i = 1; i <= frameCount; i++) {
        ...
        const backupPath = path.join(finalBackupDir, `${species}-${action}-${i}.png`);
  ...
  console.log(`✓ Backups saved to .gsd/milestones/M005/slices/S06/backup-before-90x85/`);
}
